...
def train_loop(d1, g1, g2, loader, optimizer_d1, optimizer_g1, optimizer_g2, l1, bce, g1_scaler, g2_scaler, d1_scaler):
    ...
    for idx, (x, y, y_gray) in enumerate(loader):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16):
            y_fake_gray = g1(x)
            y_fake_gray_copy = y_fake_gray.clone()
            y_fake_color = g2(y_fake_gray)
            y_fake_color_copy = y_fake_color.clone()
        ...
        d1.zero_grad()
        d1_scaler.scale(D_loss).backward(retain_graph=True)
        ...
        # Train Generator 1
        with torch.cuda.amp.autocast(dtype=torch.float16):
            D_fake_logits_gray = d1(x, y_fake_gray_copy)
            ...
            L1_gray = l1(y_fake_gray_copy, y_gray) * config.L1_LAMBDA
            G1_loss = G_fake_loss_gray + L1_gray
            ###################################################################
            
            D_fake_logits_color = d1(x, y_fake_color_copy)
            #D_fake_logits_color = torch.clamp(D_fake_logits_color, min=1e-7, max=1-1e-7)
            ###################################################################
            G2_fake_loss_color = bce(D_fake_logits_color, torch.ones_like(D_fake_logits_color))
            L1_color = l1(y_fake_color_copy, y) * config.L1_LAMBDA
        ...
        g1_scaler.scale(G1_loss).backward(retain_graph=True)
...
def main():
    ...
    test_dataset = MapDataset_Multi(sketch_dir='CUFS_Only/test_sketch', target_dir='CUFS_Only/test_photo_color')
    ...
    train_dataset = MapDataset_Multi(sketch_dir='CUFS_Only/train_sketch', target_dir='CUFS_Only/train_photo_color')
    ############################################################################
    ...
    val_dataset = MapDataset_Multi(sketch_dir='CUFS_Only/val_sketch', target_dir='CUFS_Only/val_photo_color')
    ############################################################################
