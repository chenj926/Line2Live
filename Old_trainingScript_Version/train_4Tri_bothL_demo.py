...
def main():
    ...
    gen = UnetGenerator(input_nc=3, output_nc=3, num_downs=8, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False).to(config.DEVICE)
    gen2 = UnetGenerator(input_nc=3, output_nc=3, num_downs=8, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False).to(config.DEVICE)
    gen3 = UnetGenerator(input_nc=3, output_nc=3, num_downs=8, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False).to(config.DEVICE)
    ...
    test_dataset = MapDataset_Multi(sketch_dir='Student_Only/MARK!!_sketch_test_student_DEMO', target_dir='Student_Only/MARK!!_photos_test_student_DEMO')
    ...
    #FOR DEMONSTRATION TIME, ONLY USE DATASET WITHOUT AUGMENTATION (SMALL SIZE FASTER)
    ...
    val_dataset = MapDataset_Multi(sketch_dir='Student_Only/sketch_val_student_DEMO', target_dir='Student_Only/photos_val_student_DEMO')
    ...
    for epoch in tqdm(range(config.NUM_EPOCHS)):
        ...
        if epoch == config.NUM_EPOCHS - 1:
            if not os.path.exists(f"Final_Generation/g_{name}"):
                os.makedirs(f"Final_Generation/g_{name}")
            final_save_all_Triple(gen, gen2, gen3, test_loader, folderName=f"Final_Generation/g_{name}")
            print("All Model Generations on Unseen Dataset are saved")
